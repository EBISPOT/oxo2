...
if command -v nextflow &> /dev/null; then
    ...
    echo "Making inferences using Nextflow Parallel Processing: Writing ttl took: ${elapsed} seconds"
else
    echo "Nextflow not found, using Sequential Processing of JSON to TTL conversion"
    start_time=$(date +%s)
    $SCRIPT_DIR/oxo2-json2inferences/json2ttl.sh $OXO2_DATA/sssom-as-json/mapping $OXO2_DATA/assertedMappings
    end_time=$(date +%s)
    elapsed=$((end_time - start_time))
    echo "Using Sequential Processing: Writing ttl took: ${elapsed} seconds"
fi
...
if command -v nextflow &> /dev/null; then
    echo "Using Nextflow Parallel Processing to inferring mappings"
    start_time=$(date +%s)
    nextflow -c $SCRIPT_DIR/nextflow/nextflow.config run $SCRIPT_DIR/oxo2-json2inferences/inferMappings.nf 
        end_time=$(date +%s)
        elapsed=$((end_time - start_time))
        echo "Using Nemo Parallel Processing: Nemo inferencing took: ${elapsed} seconds"           
else
    echo "Nextflow not found, using Sequential Processing of Inferring mappings"
    start_time=$(date +%s)
    ...
    if [ -s "$asserted_file" ]; then
        ...
        end_time=$(date +%s)
        elapsed=$((end_time - start_time))
        echo "Using Sequential Processing: Nemo inferencing took: ${elapsed} seconds"        
    fi
    done
fi
...
if command -v nextflow &> /dev/null; then
   echo "################### Inferred mappings to infereneces to trace and generate explanations as inference chains"
   ...
   end_time=$(date +%s)
   elapsed=$((end_time - start_time))
   echo "Using Nextflow Parallel Processing: Tracing inferences and their explanations: ${elapsed} seconds"  
else
    echo "Nextflow not found, falling back to sequential processing..."
