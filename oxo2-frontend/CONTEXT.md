...
TypeScript models (`src/model/Mapping.ts`, `MappingSet.ts`, `Search.ts`) mirror the backend response shapes.
...
The search form is extended (not a separate surface) with two optional exact-prefix selectors,
...
The search form asks one question — *which term, or terms, do you want to map?* — and offers a
...
names `score desc` itself.

There is no Advanced search surface: the per-field query tab and its flat wide results table were
removed ([ADR-0040](../docs/adr/0040-remove-advanced-search.md)) because the loaded corpora are not
metadata-rich enough to justify 40+ per-field query boxes. Per-field narrowing lives on the result
table's column filter popovers; the `advancedFieldQueries` path of `POST /api/v2/mappings/search`
remains available to API clients.
...
not component state, via the hooks in `src/util/tableUrlState.ts` (`useUrlPagination`,
`useUrlSorting`, `useUrlInferenceTypes`, `useUrlFieldFilters`, `useUrlExactFilters`,
`useUrlWeakPredicates`), used by `NormalResultsTable`.
Because the results table unmounts when a mapping's detail page opens, holding this in the URL is what restores
...
Two surfaces, two mechanisms — chosen by field **cardinality**, because a suggester over millions of
entity labels and a suggester over the handful of values of a vocabulary field are not the same problem.
...
**Picking is not typing**, and the controls model both. Both are MUI `Autocomplete` with `freeSolo`:
